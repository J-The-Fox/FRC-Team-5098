...
# take, for example, the __init__ function, which allows you to adjust variables (or properties of the house) when you create the object (house)


class revisedBlueprintForHouse():
    def __init__(self, color, size):  # ignore the self for now, just know it's necessary
        self.color = color
        self.size = size
...
class anotherBlueprintForHouse():
    ...
    # as just saying "color = newColor" would just create a new variable, or change a pre-existing variable outside of the class
    # but by using "self" the program knows that you are referring to the variable inside the class
